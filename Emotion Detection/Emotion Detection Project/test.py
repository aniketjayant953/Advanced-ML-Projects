...
# Uncomment the model you want to use and make sure the path is correct. Akso you need to re generate the model from the notebook again bc it's large
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if np.sum([roi_gray]) != 0:
            ...
            roi = cv2.resize(roi_gray, (224, 224))          # Resize to match model input
            roi = cv2.cvtColor(roi, cv2.COLOR_GRAY2RGB)  # Convert 1 channel → 3 channels (RGB)
            roi = roi.astype("float32") / 255.0
            roi = np.expand_dims(roi, axis=0)          # Shape: (1, 224, 224, 3)
